...
const perspectives = [
  ...
  { icon: Layers3, title: 'არქიტექტორის ხედვა', text: 'ნაკვეთის გეომეტრია, ლოკაცია, ურბანული რეგულაციები და საპროექტო გადაწყვეტილებისთვის საჭირო სივრცითი სიგნალები.' },
  { icon: FileSearch, title: 'ინვესტორის ხედვა', text: 'საკადასტრო, იურიდიული და მუნიციპალური რისკების სტრუქტურირებული სურათი საინვესტიციო გადაწყვეტილებამდე.' },
];
...
export default async function UrbaniaProjectPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const applicationSchema = {
    ...
    operatingSystem: 'Web',
    inLanguage: 'ka-GE',
    description: 'AI-ზე დაფუძნებული ურბანული და საკადასტრო აუდიტის სამუშაო სივრცე საქართველოში.',
    featureList: ['საკადასტრო და იურიდიული აუდიტი', 'ზონირების ანალიზი', 'K1, K2 და K3 კოეფიციენტები', 'დეველოპერის, არქიტექტორისა და ინვესტორის ხედვები'],
    provider: { '@type': 'Organization', name: 'IMI.GE', url: siteUrl },
    areaServed: { '@type': 'Country', name: 'საქართველო' },
  };
  return <div className="urbania-page relative overflow-hidden pt-32 pb-24"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(applicationSchema) }} /><div className="ai-grid pointer-events-none absolute inset-0 -z-10 opacity-50" />
