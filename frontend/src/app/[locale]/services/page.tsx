...
export default async function ServicesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="relative overflow-hidden pt-32 pb-24"><div className="ai-grid pointer-events-none absolute inset-0 -z-10 opacity-40" />
    <section className="mx-auto max-w-7xl px-6 lg:px-8"><p className="text-xs font-bold tracking-[.2em] text-cyan-700 dark:text-cyan-300">IMI.GE CAPABILITIES</p><h1 className="mt-5 max-w-4xl text-4xl font-bold leading-tight text-slate-950 dark:text-white sm:text-6xl">AI სერვისები, რომლებიც თქვენს ოპერაციულ სისტემაში ერთიანდება.</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-slate-600 dark:text-slate-300">ვმუშაობთ კონკრეტული ბიზნეს-ამოცანით, არა ტექნოლოგიის დემონსტრირებით. ყოველი მიმართულება იწყება პროცესის დიაგნოსტიკით და სრულდება გაზომვადი, უსაფრთხო production სისტემით.</p></section>
    ...
    <section className="mx-auto mt-20 max-w-7xl px-6 lg:px-8"><div className="rounded-[2rem] border border-primary/25 bg-primary/10 p-8 sm:p-12"><Bot className="text-primary"/><h2 className="mt-6 text-3xl font-bold text-slate-950 dark:text-white">არ იცით საიდან დაიწყოთ?</h2><p className="mt-4 max-w-2xl text-slate-600 dark:text-slate-300">საწყის შეხვედრაზე ვადგენთ, რომელი workflow მოიტანს შედეგს სწრაფად, რა მონაცემებია საჭირო და როგორ ავიცილოთ თავიდან არასაჭირო AI ექსპერიმენტები.</p><Link href="/contact" className="mt-8 inline-flex items-center gap-2 rounded-xl bg-slate-950 px-6 py-4 text-sm font-bold text-white dark:bg-white dark:text-slate-950">დაგეგმეთ AI roadmap <ArrowUpRight size={18}/></Link></div></section>
  </div>;
}
