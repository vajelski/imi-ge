...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl.replace(/\/$/, '')}/v1/chat/completions`, {
      ...
      body: JSON.stringify({ model, temperature: 0.65, max_tokens: 700, messages: [{ role: 'system', content: systemPrompt }, ...messages] }),
